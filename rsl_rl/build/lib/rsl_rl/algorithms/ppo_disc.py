...
class PPO_DISC:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if kwargs:
            print(
                "PPO_DISC.__init__ got unexpected arguments, which will be ignored: "
        ...
        if self.discriminator.gan_type == "wasserstein":
            self.disc_optimizer = optim.RMSprop(self.discriminator.parameters(), 
                lr=disc_learning_rate, momentum=disc_momentum, weight_decay=disc_weight_decay)
            self.disc_optimizer.zero_grad(set_to_none=True)
        else:
            self.disc_optimizer = optim.SGD(self.discriminator.parameters(), 
                lr=disc_learning_rate, momentum=disc_momentum, weight_decay=disc_weight_decay)
            self.disc_optimizer.zero_grad(set_to_none=True)
        ...
        self.disc_num_batch_size = disc_num_batch_size
        self.disc_gradient_penalty_coef = disc_gradient_penalty_coef
    ...
    def init_storage(
        ...
        self.policy_buffer = ReplayBuffer(
            num_envs=num_envs,
            num_transitions=num_transitions,
    ...
    def load_extra_components(self, motion_loader):
        self.expert_buffer = motion_loader
        motion_loader.device = self.device
    ...
    def process_disc_states(self, disc_states, next_disc_states):
        self.policy_buffer.add_transitions(disc_states, next_disc_states)
    ...
    def compute_rewards(self, states, next_states, rewards):
        self.policy_buffer.add_transitions(states, next_states)
        return self.discriminator.compute_mixed_reward(states, next_states, rewards)
    ...
    def update(self):
        ...
        policy_generator = self.policy_buffer.mini_batch_generator(self.num_mini_batches, self.num_learning_epochs, self.disc_num_batch_size)
        expert_generator = self.expert_buffer.mini_batch_generator(self.num_mini_batches, self.num_learning_epochs, self.disc_num_batch_size)
        
        self.train_mode()
        kl_sum_list = []
        for rollout_samples, policy_samples, expert_samples in zip(generator, policy_generator, expert_generator):
            ...
            policy_states_batch, next_policy_states_batch = policy_samples
            expert_states_batch, next_expert_states_batch = expert_samples

            self.actor_critic.act(obs_batch, commands_batch, hidden_states=hidden_a_batch)
            actions_log_prob_batch = self.actor_critic.get_actions_log_prob(actions_batch)
            values_batch = self.actor_critic.evaluate(critic_obs_batch, critic_commands_batch, hidden_states=hidden_c_batch)
            ...
            # Discriminator loss.
            policy_prediction = self.discriminator.compute_prediction(policy_states_batch, next_policy_states_batch).mean()
            policy_loss = self.discriminator.compute_policy_loss(policy_states_batch, next_policy_states_batch)
            expert_loss = self.discriminator.compute_expert_loss(expert_states_batch, next_expert_states_batch)
            grad_penalty = self.discriminator.compute_grad_penalty(expert_states_batch, next_expert_states_batch)
            
            discriminator_loss = policy_loss + expert_loss + self.disc_gradient_penalty_coef * grad_penalty
            (discriminator_loss / self.num_mini_batches).backward()
            ...
            mean_disc_pred += policy_prediction.item()
        ...
        mean_disc_pred /= num_updates
        mean_kl_divergence /= num_updates
        self.storage.clear()

        return mean_value_loss, mean_surrogate_loss, mean_estimator_loss, mean_kl_divergence, mean_disc_loss, mean_disc_pred
